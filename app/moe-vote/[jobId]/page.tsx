            ...
            <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center">
                        ...
                        <p className="mt-4 text-gray-600 dark:text-gray-400">正在加载投票结果...</p>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
                        ...
                        <p className="text-red-700 dark:text-red-400">{error || '未找到任务'}</p>
                        <button
                            onClick={() => router.push('/')}
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">混合专家讨论结果</h1>
                </div>

                {/* Question Card */}
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
                    <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">问题</h2>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        >
                            {job.status === 'completed' ? '已完成' : job.status === 'failed' ? '失败' : job.status === 'running' ? '运行中' : '等待中'}
                        </span>
                    </div>
                    ...
                    {job.progress && (
                        <div className="mb-4">
                            <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-2">
                                <span>
                                    第 {job.progress.currentRound} / {job.progress.totalRounds} 轮
                                </span>
                                <span>{formatPhase(job.progress.phase)}</span>
                            </div>
                            ...
                            <div className={`rounded-lg p-8 border-2 ${job.result.finalDecision === 'yes'
                                ...
                                <div className="text-center">
                                    ...
                                    <p className={`text-4xl font-bold mb-3 ${job.result.finalDecision === 'yes'
                                        ...
                                        {job.result.finalDecision === 'yes' ? '是' : '否'}
                                    </p>
                                    <p className="text-lg text-gray-600 dark:text-gray-400">
                                        置信度：{job.result.confidence}%
                                    </p>
                            ...
                            <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 rounded-lg p-6 border-2 border-yellow-300 dark:border-yellow-700">
                                <div className="text-center">
                                    ...
                                    <p className="text-lg text-gray-600 dark:text-gray-400 mt-2">
                                        平均分：<span className="font-semibold">{job.result.winner.averageScore.toFixed(2)}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="text-center">
                                        ...
                                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">轮次</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-3xl font-bold text-green-600 dark:text-green-400">
                                            {job.result.discussionSummary.totalMessages}
                                        </div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">消息</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                                            {job.result.discussionSummary.toolCallsUsed}
                                        </div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">工具调用</div>
                                    </div>
                                </div>
                            </div>
                        )}
                ...
                <div className="mt-6 text-sm text-gray-500 dark:text-gray-400 text-center">
                    <p>任务 ID：{job.jobId}</p>
                    <p>创建时间：{new Date(job.createdAt).toLocaleString()}</p>
                    {job.completedAt && <p>完成时间：{new Date(job.completedAt).toLocaleString()}</p>}
                </div>
            </div>
        </div>
    );
